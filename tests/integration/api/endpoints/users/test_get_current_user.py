...
class TestWhenVisitingByRegisteredUser:
    ...
    @pytest.fixture
    async def anonymous_response(self, registered_user_client: AsyncClient) -> Response:
        response = await registered_user_client.get("/user")
        return response

    @pytest.mark.anyio
    async def test_returns_status_200_OK(self, anonymous_response: Response) -> None:
